        ...
        <div class="mt-10 px-4 sm:mt-16 sm:px-0 lg:mt-0">
            ...
            <div class="flex items-center">
              <div class="flex items-center">
                <StarIcon v-for="rating in [0, 1, 2, 3, 4]" :key="rating"
                  :class="[product.rating > rating ? 'text-Secondary' : 'text-gray-300', 'h-6 w-6 flex-shrink-0']"
                  aria-hidden="true" />
              </div>
          ...
          <form class="mt-6">
            ...
            <div class="mt-10 flex" v-if="false">
              <button type="submit"
                class="flex max-w-xs flex-1 items-center justify-center rounded-md border border-transparent bg-primary px-8 py-3 text-base font-medium text-white hover:bg-teal-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50 sm:w-full">Add
                to bag</button>
          ...
          <section aria-labelledby="details-heading" class="mt-12">
            ...
            <div class="divide-y divide-gray-200 border-t">
              <Disclosure as="div" v-for="(detail, inx) in product.details" :key="detail.name" v-slot="{ open }"
                :defaultOpen="inx === 0">
                <h3>
                  <DisclosureButton class="group relative flex w-full items-center justify-between py-6 text-left">
                    <span :class="[open ? 'text-primary' : 'text-gray-900', 'text-sm font-medium']">{{ detail.name
                      }}</span>
                    <span class="ml-6 flex items-center">
                      <PlusIcon v-if="!open" class="block h-6 w-6 text-gray-400 group-hover:text-gray-500"
                        aria-hidden="true" />
                      <MinusIcon v-else class="block h-6 w-6 text-primary group-hover:text-primary"
                        aria-hidden="true" />
                    </span>
                  </DisclosureButton>
                </h3>
